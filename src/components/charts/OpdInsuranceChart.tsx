...
interface OpdInsuranceChartProps {
  data: InsuranceGroup[]
  isLoading: boolean
}

const COLORS = ['#3b82f6', '#ef4444', '#10b981', '#f97316', '#8b5cf6', '#f59e0b', '#ec4899']

export function OpdInsuranceChart({ data, isLoading }: OpdInsuranceChartProps) {
  return (
    <Card className="h-full">
      ...
      <CardContent className="pt-5">
        {isLoading ? <Skeleton className="h-[460px] w-full" /> : data.length === 0 ? <EmptyState title="ไม่มีข้อมูลสิทธิการรักษาของผู้ป่วยนอก" /> : (
          <ResponsiveContainer width="100%" height={460}>
            <PieChart>
              <Pie
                ...
                labelLine={false}
              >
                {data.map((item, index) => <Cell key={item.rightName} fill={COLORS[index % COLORS.length]} />)}
              </Pie>
